...
class Read(vr: Variable, r: Option[Int]) 
  extends ReadInstruction(vr, r) 
  with Instruction(vr) {
...
class Write(vw: Variable, val wConcrete: Int) 
  extends WriteInstruction(vw, Some(wConcrete)) 
  with Instruction(vw) {
  override def toString(): String = s"(wr: $v : ${w.get})"
}

class ReadWrite(vr: Variable, r: Option[Int], vw: Variable, w: Option[Int], val compute: Int => Int) 
  extends ReadInstruction(vr, r)
  with WriteInstruction(vw, w)
  with Instruction(vw) {
...
class Lock(val vl: Variable) 
  extends ReadWrite(vl, 0, 1) {
  override def toString(): String = s"(lk: $v)"
}
class Unlock(val vl: Variable) 
  extends ReadWrite(vl, 1, 0) {
